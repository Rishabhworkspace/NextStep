    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-[#FAFAFA] pt-20 pb-12 px-4">
      ...
      <div className="w-full max-w-md bg-white/70 backdrop-blur-2xl border border-white p-8 sm:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] relative z-10 flex flex-col">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="text-3xl font-heading font-extrabold text-[#111827] tracking-tight mb-2">Start your journey</h1>
          <p className="text-[#4B5563] text-sm">Create an account to build your roadmap.</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <form className="space-y-4" onSubmit={handleEmailSignup}>
            ...
            <Button type="submit" className="w-full h-11 text-base bg-[#111827] text-white hover:bg-black rounded-xl group transition-all mt-2" disabled={loading}>
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Create Account <ArrowRight className="w-4 h-4 ml-2 opacity-70 group-hover:translate-x-1 group-hover:opacity-100 transition-all" /></>}
            </Button>
          </form>
        </div>
        
        <p className="mt-8 text-center text-sm text-[#4B5563]">
          Already have an account? <Link href="/auth/login" className="text-orange-500 hover:text-orange-600 hover:underline font-semibold transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  )
